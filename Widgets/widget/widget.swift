       ...
       ZStack {
         ...
         VStack(alignment: .leading){
           Text(entry.city).foregroundColor(Color.init(hex: 0x5686DF)).font(.system(size: 15)).fontWeight(.medium).padding(.leading,3)
            ...
            HStack{
              ForEach(entry.wel?.daily[..<4] ?? [],id: \.self){daily in
                 VStack{
...
struct widget: Widget {
    ...
    var body: some WidgetConfiguration {
        IntentConfiguration(kind: kind, intent: ConfigurationIntent.self, provider: Provider()) { entry in
          widgetEntryView(entry: entry).background(Color.green)
        }
...
struct widget_Previews: PreviewProvider {
    static var previews: some View {
        widgetEntryView(entry: SimpleEntry(date: Date(), configuration: ConfigurationIntent(),wel: nil,city: "Beijing"))
